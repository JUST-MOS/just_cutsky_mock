...
class SkyRegion:
    """Sky selection used by the lightcone builder.

    Two modes are supported:
      * ``rectangle``: RA/Dec limits in degrees. RA wrap-around is supported.
    ...
    def contains(self, ra_deg: np.ndarray, dec_deg: np.ndarray) -> np.ndarray:
        ...
        if self.mode == "rectangle":
            ...
            elif self.ra_min <= self.ra_max:
                in_ra = (ra >= self.ra_min) & (ra <= self.ra_max)
            else:
                in_ra = (ra >= self.ra_min) | (ra <= self.ra_max)
            return in_ra & (dec >= self.dec_min) & (dec <= self.dec_max)

        ra0 = np.radians(self.center_ra)
        dec0 = np.radians(self.center_dec)
        rar = np.radians(ra)
        decr = np.radians(dec)
        cosang = (
            np.sin(decr) * np.sin(dec0)
            + np.cos(decr) * np.cos(dec0) * np.cos(rar - ra0)
        )
        radius = np.radians(self.angular_radius_deg)
        return np.arccos(np.clip(cosang, -1.0, 1.0)) <= radius
    ...
    def bounding_cap(self) -> tuple[float, float, float]:
        ...
        # For very wide rectangles an enclosing small cap does not exist; using
        # a full-sky cap disables angular tile pruning but remains exact.
        if width > 180.0:
...
class MockConfig:
    ...
    def __post_init__(self):
        ...
        object.__setattr__(self, "nyu_vagc_path", Path(self.nyu_vagc_path).expanduser().resolve())
