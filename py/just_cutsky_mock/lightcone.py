...
def candidate_tiles(
    ...
    *,
    ...
    """Find periodic replicas that can intersect the requested HEALPix volume."""
    n = int(np.ceil(dmax / box_size)) + 1
    cap_ra, cap_dec, cap_radius_deg = geometry.bounding_cap()
...
def _radial_bounds_for_observed_redshift(
    galaxies: GalaxyBox,
    distances: DistanceTable,
    z_min: float,
    z_max: float,
) -> tuple[float, float, float, float]:
    """Conservative comoving-distance bounds for a final z_obs selection.

    The exact RSD relation used below is
        1 + z_obs = (1 + z_com) / (1 - beta),
    with beta = v_radial/c. Since |v_radial| <= |v|, the largest galaxy speed
    gives a conservative beta bound and therefore an exact preselection shell.
    """
    if galaxies.size == 0:
        return 0.0, 0.0, z_min, z_max

    max_speed = float(np.max(np.linalg.norm(galaxies.vel, axis=1)))
    beta_max = min(max_speed / C_KM_S, 0.999999)

    zcom_min = max(0.0, (1.0 + z_min) * (1.0 - beta_max) - 1.0)
    zcom_max = (1.0 + z_max) * (1.0 + beta_max) - 1.0

    # DistanceTable is built beyond z_max; extend only in the extremely unusual
    # case that the velocity-derived conservative z_com upper bound exceeds it.
    if zcom_max > distances.z_samples[-1]:
        raise ValueError(
            f"Velocity-derived z_com upper bound ({zcom_max:.3f}) exceeds the distance table "
            f"({distances.z_samples[-1]:.3f})."
        )

    return (
        float(distances.dcom_h_from_z(zcom_min)),
        float(distances.dcom_h_from_z(zcom_max)),
        zcom_min,
        zcom_max,
    )
...
def _process_tile(
    ...
    angular = geometry.contains(ra, dec)
...
def build_galaxy_lightcone(
    ...
    *,
    ...
    z_max: float,
    n_jobs: int,
) -> tuple[dict[str, np.ndarray], DistanceTable, dict[str, float]]:
    # Build farther than z_max so the velocity-derived z_com shell always fits
    # ordinary peculiar velocities without relying on a hard-coded Delta-z.
    distances = DistanceTable.build(h0, om0, max(z_max + 1.0, z_max * 1.5))
    radial_dmin, radial_dmax, zcom_min, zcom_max = _radial_bounds_for_observed_redshift(
        galaxies, distances, z_min, z_max
    )
    tiles = candidate_tiles(geometry, box_size=box_size, dmin=radial_dmin, dmax=radial_dmax)
    ...
    diagnostics = {
        "radial_dmin_Mpc_h": radial_dmin,
        "radial_dmax_Mpc_h": radial_dmax,
        "zcom_preselect_min": zcom_min,
        "zcom_preselect_max": zcom_max,
        "n_candidate_tiles": len(tiles),
    }

    results = [r for r in results if len(r["z_obs"]) > 0]
    if not results:
        return _empty_galaxy_lightcone(), distances, diagnostics

    keys = ["index", "type", "ra", "dec", "x", "y", "z", "vx", "vy", "vz", "z_obs", "z_com", "Mh", "Ms", "Mr", "mr", "gr", "tile"]
    return {k: np.concatenate([r[k] for r in results], axis=0) for k in keys}, distances, diagnostics
...
def apply_evolution(lightcone: dict[str, np.ndarray], *, random_seed: int) -> dict[str, np.ndarray]:
    ...
    out = {key: value[keep] for key, value in lightcone.items()}
...
def build_corresponding_halo_lightcone(
    ...
    *,
    ...
    Row i is the host halo of galaxy row i. Host halos intentionally repeat
    when several selected galaxies share the same host. A host center can fall
    just outside the selected HEALPix union when a satellite lies just inside a
    boundary pixel; it is retained because correspondence is the defining rule.
    """
