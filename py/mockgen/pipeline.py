...
def generate_mock(config: MockConfig) -> MockResult:
    ...
    print("[1/5] Loading host halo catalog...")
    halos = load_uchuu_catalog(config.uchuu_path)
    print(f"      {halos.size:,} halos")

    print("[2/5] Populating the periodic box with the fixed HOD model...")
    ...
    print("[3/5] Assigning halo quenching, g-r color, and Mr from bundled calibration...")
    calibration = load_calibration()
    galaxy_box = assign_galaxy_properties(
        galaxy_box,
        calibration,
        random_seed=config.random_seed,
    )

    print("[4/5] Building requested cutsky lightcone and applying evolution...")
    ...
    print("[5/5] Building row-aligned host halos and writing mock.h5...")
